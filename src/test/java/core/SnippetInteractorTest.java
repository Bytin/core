...
public class SnippetInteractorTest {
    ...
    public void setUp() {
        ...
        snippetOwners = new String[] {"noah", "kate", "andy"};
    ...
    void createUsers() {
        for (String owner : snippetOwners) {
            userInteractor.createUser(new CreateUser.RequestModel(owner, "asdlfkwe", "a@g.com"));
        }
    }

    void createSnippets() {
        boolean[] hidden = new boolean[] {false, true, true};

        for (int i = 0; i < snippetOwners.length; i++) {
            SnippetDTO snippetDTO = SnippetDTO.builder().id(i + 1).title("test").language("java")
                    .framework(null).code("assert").description("rigor").resource(null)
                    .owner(snippetOwners[i]).hidden(hidden[i])
    ...
    void createSnippetWithBadRequests() {
        var requestWithNullTitle = new CreateSnippet.RequestModel(SnippetDTO.builder().title(null)
                .language("java").framework(null).code("assert").description("rigor").resource(null)
        ...
        var requestWithNullDateModified = new CreateSnippet.RequestModel(
                SnippetDTO.builder().title("").language("java").framework(null).code(null)
                        ...
                        .whenCreated(LocalDateTime.of(2020, 12, 5, 0, 0, 0)).whenLastModified(null)
                        .build());
